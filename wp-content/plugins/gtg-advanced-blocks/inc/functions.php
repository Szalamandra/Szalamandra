...
if ( ! function_exists( 'gtg_timeline_get_classes' ) ) {
	/**
	 * Function Name: gtg_timeline_get_classes .
	 *
...
if ( ! function_exists( 'gtg_timeline_get_align_classes' ) ) {
	/**
	 * Function Name: gtg_timeline_get_align_classes description.
	 *
...
if ( ! function_exists( 'gtg_timeline_get_day_align_classes' ) ) {
	/**
	 * Function Name: gtg_timeline_get_day_align_classes description.
	 *
	 * @param array  $attributes attribute array.
	 * @param string $index_val  post index.
	 * @return string            output HTML/String.
...
if ( ! function_exists( 'gtg_get_custom_icons' ) ) {
	function gtg_get_custom_icons() {
		global $wpdb;
		$icon_options = get_option( Gtg_Advanced_Blocks\Modules\CustomIcons::OPTION_NAME, [] );
		//
		$custom_icons = [];
		$query = $wpdb->prepare("
			SELECT * FROM {$wpdb->prefix}gtg_block_icons WHERE icon_status = %s
		", 'publish');

		$icons = $wpdb->get_results( $query, ARRAY_A );
		if ( $icons ) {
			foreach ( $icons as $icon ) {
				if ( ! empty( $icon['icon_dir_name'] ) && isset( $icon_options[$icon['icon_dir_name']] ) ) {
					$custom_icons[$icon['icon_dir_name']] = $icon_options[$icon['icon_dir_name']];
				}
			}
		}
...
if ( ! function_exists( 'gtg_minify_css' ) ) {
	/**
	 * Minify CSS
	 *
	 * @param string $css
	 ...
	 * @since 1.1.2
	 */
	function gtg_minify_css( $css = '' ){
	  $css = str_replace( array( "\r\n", "\r", "\n", "\t", '  ', '    ', '    ' ), '', $css );
	  return $css;
	}
}

if ( ! function_exists( 'gtg_sanitize_boolean' ) ) {

	/**
	 * sanitize string to boolean
	 *
	 * @param mixed $value
	 * @return boolean
	 * @since 1.1.2
	 */
	function gtg_sanitize_boolean( $value = '' ) {
		if ( is_string( $value ) ) {
