...
class Ajax {
	...
	public static function import_template() {
		try {
			...
			$import_setting = ! isset( $_REQUEST['import-setting'] ) ? (bool)$_REQUEST['import-setting'] : false;
			$create_new_page = isset( $_REQUEST['create-new-page'] );
			$current_kit = isset( $_REQUEST['current-kit'] ) ? array_map( 'sanitize_text_field', $_REQUEST['current-kit'] ) : false;
			...
			if ( $local_id ) {
				...
				if ( 200 !== $header_status ) {
					...
					if ( ! empty( $response['data'] ) && $name ) {
						if ( ! gtg_is_remote_item_saved( $id ) || $create_new_page ) {
							...
							$args = [
								'post_title' => wp_specialchars_decode( $name ),
							...
							$args = apply_filters( 'gutengeek_import_block_insert_args', $args );
							$args = wp_slash( $args );
							$args['post_title'] = wp_specialchars_decode( $args['post_title'] );
							$local_id = wp_insert_post( $args );
							...
							if ( $current_kit && ! empty( $current_kit['name'] ) ) {
								update_post_meta( $local_id, '_gutengeek_kit_name_imported', $current_kit['name'] );
							}

							if ( ! empty($response['template_settings']) && ( $args['post_type'] === 'wp_block' || ( $import_setting && $create_new_page ) ) ) {
								// update meta key _gutengeek_post_settings
								update_post_meta( $local_id, '_gutengeek_post_settings', $response['template_settings'] );
							}

							if ( $create_new_page ) {
								$response['local_edit_link'] = admin_url( 'post.php?post=' . $local_id . '&action=edit' );
							}
						}
						$response['id'] = $id;
					}
