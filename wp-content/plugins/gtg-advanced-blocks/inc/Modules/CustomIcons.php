...
class CustomIcons {
	...
	public static function get_supported_icon_sets() {
		$icon_sets = [
			...
			'svg' => 'Gtg_Advanced_Blocks\Modules\Icon_Sets\Svg',
		];
	 ...
	 * import json file
	 */
	public static function import_json_file( $file = false ) {
		global $wpdb;
		global $wp_filesystem;
	    require_once( ABSPATH . 'wp-admin/includes/file.php' );
		...
		if ( ! file_exists( $file ) && ! file_exists( trailingslashit( ABSPATH ) . $file ) ) {
			return new \WP_Error( 'file_not_exists', __( 'No custom icons file found', 'gutengeek' ) );
		}

		$file = file_exists( $file ) ? $file : trailingslashit( ABSPATH ) . $file;
		...
		if ( $exist ) {
			...
			if ( ! $status ) {
				return new \WP_Error( 'import_failed', __( 'Import custom font failed', 'gutengeek' ) );
			}
		...
		if ( $icon_id ) {
			$custom_icons = self::get_options();
			$json_data['id'] = $icon_id;
			$custom_icons[$json_data['key_name']] = $json_data;
			self::update_options( $custom_icons );
